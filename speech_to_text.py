...
def convert(filename):
    AUDIO_FILE = path.join(path.dirname(path.realpath(__file__)), "file_" + str(filename) + ".wav")
    ...
    try:
        ...
        f = open('out_' + str(filename) + '.txt', 'w')
        sys.stdout = f
        ...
        pprint(r.recognize_google(audio, show_all=False,language="en-IN"))  # pretty-print the recognition result
...
GOOGLE_CLOUD_SPEECH_CREDENTIALS = rINSERT THE CONTENTS OF THE GOOGLE CLOUD SPEECH JSON CREDENTIALS FILE HERE
try:
